...
El software posee la capacidad de leer indicadores de hardware y el estado de integridad de los discos duros y SSDs instalados en el dispositivo. Esto posibilita la identificación temprana de fallos físicos (sectores defectuosos) y degradación del almacenamiento.
...
Y rellene el campo "Usuario WMI" con cualquier información (se descartará más adelante) y el campo "Contraseña WMI" con la contraseña indicada durante la instalación de la sonda.
...
| `--port` | Informa el puerto que será utilizado por la sonda colectora. Si no se indica, el valor por defecto será 7744 (TCP). |
| `--passwd` | Asigna la contraseña que será utilizada por la sonda colectora. La contraseña por defecto será *monsta@dm* si no se proporciona. |
