...
sidebar:
  order: 5
---
A **Sonda Monsta** es un software de recopilación local diseñado para instalarse directamente en servidores y dispositivos **Windows, Linux e Raspberry PI**. Su función principal es recopilar métricas de rendimiento, integridad y disponibilidad del sistema anfitrión, funcionando como una extensión nativa de recopilación para la plataforma Monsta.

## Características y Capacidades Técnicas

### 1. Arquitectura Pasiva (Bajo Demanda)

La sonda opera estrictamente bajo un modelo **pasivo de petición y respuesta**. No inicia comunicaciones en la red de forma autónoma; el tráfico de datos ocurre solo cuando Monsta se pone en contacto para realizar el *polling* (solicitud de recopilación).
...
- **Scripts PowerShell:** Soporta el lanzamiento de scripts personalizados, permitiendo supervisar aplicaciones específicas o crear rutinas de validación a medida.
...
2. Con sesión iniciada con un usuario administrador, ejecute el instalador "monstaprobe.exe" (consulte [Instalación por línea de comandos](#instalação-pela-linha-de-comando) para instalación en lote);
...
| `--port` | Indica el puerto a ser utilizado por la sonda colectora. Si no se indica, el predeterminado será 7743 (TCP). |
| `--passwd` | Asigna la contraseña a ser utilizada por la sonda colectora. La contraseña predeterminada será *monsta@dm* si no se indica. |
...
:::note
**port**: Es el puerto que será utilizado por la sonda para que Monsta se conecte. El predeterminado es **7743** (TCP).  
...
Y rellene el campo "Usuario WMI" con cualquier información (será descartada posteriormente) y el campo "Contraseña WMI" con la contraseña indicada en la instalación de la sonda.
