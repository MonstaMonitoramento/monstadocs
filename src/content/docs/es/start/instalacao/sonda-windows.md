...
El software posee la capacidad de leer indicadores de hardware y el estado de integridad de los discos duros y SSD instalados en el dispositivo. Esto posibilita la identificación temprana de fallos físicos, como bloques defectuosos (*bad blocks*), y la degradación del almacenamiento.
...
| Opción &nbsp; &nbsp; &nbsp; &nbsp; | Descripción |
| --------------------------------- | -------------------------------------------------------------------------------------------------------- |
| `AGREE=Y` | Acepta el acuerdo de uso de la sonda colectora. |
| `PORT=<num>` | Indica el puerto que será utilizado por la sonda colectora. Si no se indica, el valor predeterminado será 7743 (TCP). |
| `PASSWD=<password>` | Asigna la contraseña que utilizará la sonda colectora. |
...
| `--port` | Redefine el puerto en el que la sonda deberá escuchar. |
...
Y rellene el campo "Usuario WMI" con cualquier información (será descartada más adelante) y el campo "Contraseña WMI" con la contraseña indicada durante la instalación de la sonda.
