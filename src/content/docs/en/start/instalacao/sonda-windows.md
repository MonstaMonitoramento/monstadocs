...
The software has the ability to read hardware indicators and the health status of hard drives and SSDs installed on the device. This enables early identification of physical failures (*bad blocks*) and storage degradation.

### 5. Encrypted Communication

All information exchange between the Monsta central server and the Probe installed on the device is **100% encrypted**, ensuring the security of transmitted metrics and preventing interception of sensitive infrastructure data.
...
The MonstaProbe.exe installer accepts command line options. You can use them to automate installation across a network via a GPO, without interaction with the graphical interface.
